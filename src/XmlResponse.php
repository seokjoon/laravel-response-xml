...
class XmlResponse
{
	...
	private function rowName($row, $depth = 0)
	{
		static $depthArray = [];
		if($depth > 0) array_push($depthArray, $depth);
		$diff = $depth - min($depthArray); //Log::info($diff);

		if (!empty($this->rowName)) return $this->rowName[$diff];
		else return 'row_' . $row;
	}
	////////
	...
	public function array2xml($array, $xml = false, $config = [], $status = 200)
		//public function array2xml($array, $xml = false, $config = [], $status = 200, $depth = 0)
		////////
	{
		...
		foreach ($array as $key => $value) {
			...
			if (is_array($value)) {
				if (is_numeric($key)) {
					...
					$this->array2xml($value, $xml->addChild($this->caseSensitive($this->rowName($key, $depth))));
					////////
				} else {
				...
				if (!is_null($value) || $this->showEmptyField) {
					if (is_numeric($key)) {
						...
						$depth = count(debug_backtrace());
						$xml->addChild($this->caseSensitive($this->rowName($key, $depth)), htmlspecialchars($value));
						////////
					} else {
